...
fun generateFractalTreePrecomputed(
    ...
    fun generateRecursive(
        ...
        val referenceVector = if (abs(unitDirection.dot(Vector(0, 1, 0))) > 1.0 - epsilon) Vector(1, 0, 0) else Vector(0, 1, 0)
        ...
        val newDirectionA = rotateVectorDebug(rotateVectorDebug(unitDirection.clone(), axis1, branchAngleA), axis2, branchAngleB).multiply(newLength)
        val newDirectionB = rotateVectorDebug(rotateVectorDebug(unitDirection.clone(), axis1, -branchAngleA), axis2, -branchAngleB).multiply(newLength)
